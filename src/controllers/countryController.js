...
exports.saveCountry = async (req, res) => {
	try {
		...
		if (savedCountry)
			return res.status(statusCodes.created).json({ message: messages.countrySaved, user: _copy(savedCountry) });
		return res.status(statusCodes.failed).json({ message: messages.countryNotSaved });
	} catch (e) {
		if (e && e.code === 11000) {
			if (e.keyPattern && e.keyPattern.slug)
				return res.status(statusCodes.success).json({ message: messages.countryAlreadyExists });
			return res.status(statusCodes.success).json({ message: messages.countryExistsByCode });
		}
