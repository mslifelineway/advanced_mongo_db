...
exports.validatePhoneNumberWithCountry = (phoneNumber, country, format = 'international') => {
	const { error, value } = joiPhoneNumber
		.string()
		.phoneNumber({ defaultCountry: `${country}`, format: `${format}`, strict: true })
		.validate(`${phoneNumber}`);

	return { error: error ? errors.invalidPhoneNumber : undefined, phone_number: value };
};
